...
public class GaussianKernel extends Kernel {
    ...
    public void init(Dataset nd, double bandwidth) {
        this.bandwidth = bandwidth;
        sigma = new double[nd.getDimension()][nd.getDimension()];
        for (int i = 0; i < sigma.length; i++) {
            sigma[i][i] = bandwidth * bandwidth;
        }
        invSigma = Algebra.DEFAULT.inverse(new DenseDoubleMatrix2D(sigma)).toArray();
        normConst = normConst = 1 / Math.sqrt(Math.pow(2 * Math.PI, nd.getDimension()) * Algebra.DEFAULT.det(new DenseDoubleMatrix2D(sigma)));
        this.ready = true;
    }
    ...
    public double getValue(double[] center, double[] vector) {
        ...
        double val = normConst * getNonNormalizedValue(center, vector);
        //if(Math.random()<1000)logger.print(val/MultiNormalDist.density(center, sigma, vector));
        return val;
    }
    ...
    protected double getNonNormalizedValue(double[] center, double[] vector) {
        ...
        if (invSigma != null) {
            return getNonNormalizedValueCov(center, vector);
        }
        double diff = 0;
        for (int i = 0; i < vector.length; i++) {
            diff += (center[i] - vector[i]) * (center[i] - vector[i]);
        }
    ...
    public BigDecimal getDensity(double[] center, double[][] vectors) {
        ...
        for (int i = 0; i < vectors.length; i++) {
            BigDecimal exp = new BigDecimal(getNonNormalizedValue(center, vectors[i]), MathContext.DECIMAL128);
