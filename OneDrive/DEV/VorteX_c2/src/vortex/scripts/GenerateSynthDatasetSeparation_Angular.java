...
public class GenerateSynthDatasetSeparation_Angular {
    ...
    public static void main(String[] args) {
        ...
        Boolean [] sparsity_vector = new Boolean[dim-1];
        
        for (int i = 0; i < sparsity_vector.length; i++) {
            sparsity_vector[i] = i < effective_num_dim;
        }
        
        (new Shuffle<Boolean>()).shuffleArray(sparsity_vector);
        ...
        for (int j = 0; j < dim; j++) {
            boolean sparse = !(j > 0?sparsity_vector[j-1]:true);
            for (int i = 0; i < 2; i++) {
                if (sparse) {
                    mu[i].setQuick(j, 0);
