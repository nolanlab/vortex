...
public class GabrielNeighborTaskGeneric extends GabrielNeighborTaskAngular {
    ...
    public int[] execute() {
        ...
        int maxIdx = Math.max(sorted.size(), ds.getDimension() * 100);
        ...
        for (int y = 0; y < maxIdx; y++) {
            ...
            double[] anotherVector = anotherDP.getVector();

            mid = dm.getPrototype(new double[][]{center, anotherVector});

            double simBetween = Math.min(dm.getSimilarity(mid, center), dm.getSimilarity(mid, anotherVector));
//            assert MatrixOp.mult(mid, center) == Kernel.getEuclideanCosine(new DenseDoubleMatrix1D(mid), new DenseDoubleMatrix1D(center));
            ...
            if (YisNeighbor && nnOfTheMid != null) {
                ...
                b = dm.getSimilarity(anotherVector, midShiftedToNN);
