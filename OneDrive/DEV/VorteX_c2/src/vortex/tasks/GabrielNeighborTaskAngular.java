...
public class GabrielNeighborTaskAngular implements ReusingTask<int[]> {
    ...
    /*
     * Returns the set of IDs of gabriel neighbors of sortedUnityLenVectors[0] in the array of sortedUnityLenVectors
    ...
    public int[] execute() {
        ...
        int maxIdx = Math.min(ds.getDimension() * 25, sorted.size());
        ...
        for (int y = 0; y < maxIdx; y++) {
            ...
            double[] anotherVector = anotherDP.getUnityLengthVector();

            mid = MatrixOp.toUnityLen(MatrixOp.sum(center, anotherVector));

            double cosBetween = Math.min(1.0, Math.min(MatrixOp.mult(mid, center), MatrixOp.mult(mid, anotherVector)));
//            assert MatrixOp.mult(mid, center) == Kernel.getEuclideanCosine(new DenseDoubleMatrix1D(mid), new DenseDoubleMatrix1D(center));
            ...
            if (YisNeighbor && nnOfTheMid != null && trim_alpha > 0.000001) {
                ...
                b = Math.min(1.0, MatrixOp.mult(anotherVector, midShiftedToNN));
