...
public class MinimumSpanningTree {
    ...
    public MinimumSpanningTree(final double[][] edgeLenghMatrix) {
        ...
        System.out.println("finding the threshold");
        int comp = 0;
        for (int optRound = 0; optRound < 10; optRound++) {
            System.out.println("OptRound " + optRound + "min=" + min + ", max=" + max);
        ...
        System.out.println("Edge list to array");
        ...
        System.out.println("Doing one-by-one addition, total edges: " + edges.length);
        ...
        for (int i = 0; i < edges.length; i++) {
            ...
            if (areConnected(edges[i][0], edges[i][1], edgeMtx, idxVisited, nextLayer)) {
                ...
                for (int j = i + 1; j < edges.length; j++) {
                    if (edges[j] == null) {
                        //System.out.println("edges["+j+"] is null");
                        continue;
                    }
                ...
                System.out.println("is a part of the MST:" + Arrays.toString(edges[i]));
            }
        }
        System.out.println("Found edges in the MST:" + edgeList.size());
        edges = edgeList.toArray(new int[edgeList.size()][]);
    }
    ...
    private boolean[] getConnectedComponentOf(int source, boolean[][] mtx, boolean[] hasBeenVisited, boolean[] nextLayer) {
        ...
        //System.out.println("areConnected: " + source + ", " + dest);
        //System.out.println("nextLayer[source]= " + nextLayer[source]);
        do {
            ...
            while (it.hasNext()) {
                ...
                for (int i = 0; i < dim; i++) {
                    ...
                    if ((mtx[idx][i] || mtx[i][idx]) && !hasBeenVisited[i]) {
                        //           System.out.println("nextLayer:" + i);
                        nextLayer[i] = true;
    ...
    private int getNumConnectedComponents(boolean[][] mtx, boolean[] idxVisited) {
        ...
        do {
            ...
            do {
                ...
                //System.out.println("isGraphConnected round#"+(++cnt) + " currLayer"+ currLayer.size());
            } while (!currLayer.isEmpty());
