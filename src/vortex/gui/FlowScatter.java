...
public class FlowScatter extends JPanel {
    ...
    public BufferedImage generateScatterImage(Series s) {
        ...
        maxSumBin = Math.min(maxSumBin, 3);
        for (int x = 0; x < w; x++) {
            for (int y = 0; y < h; y++) {
                ...
                if (binnedDS[x][y] > 0) {
                    g.setColor(new Color(s.c.getRed(), s.c.getGreen(), s.c.getBlue(), s.c.getAlpha()));
                    g.fillOval(x, y, 2, 2);
                }
            }
        }
    ...
    public BufferedImage generateDensityImage(Series s) {
        ...
        for (int x = 0; x < w; x++) {
            for (int y = 0; y < h; y++) {
                if (convovled[x][y] > 0) {
                    g.setColor(new Color(s.c.getRed(), s.c.getGreen(), s.c.getBlue(), (int) ((convovled[x][y] / maxSumW) * 255)));
                    g.fillRect(x, y, 1, 1);
                }
            }
        }
    ...
    public BufferedImage generateContourImage(Series s, int numContours) {
        ...
        if (filled) {
            for (int x = 0; x < w; x++) {
                for (int y = 0; y < h; y++) {
                    if (convovled[x][y] > 0) {
                        g.setColor(new Color(s.c.getRed(), s.c.getGreen(), s.c.getBlue(), (int) (((int) (convovled[x][y] * 10)) * 20)));
                        g.fillOval(x, y, 1, 1);
                    }
                }
            }
        ...
        for (int x = 0; x < w; x++) {
            for (int y = 0; y < h; y++) {
                ...
                if (binnedDS[x][y] > 0 && convovled[x][y] < stepSize) {
                    g.setColor(new Color(s.c.getRed(), s.c.getGreen(), s.c.getBlue(), 200));
        ...
        for (ArrayList<GeneralPath> paths : pathsByThs) {
         ...
         for (int x = 0; x < w; x++) {
            for (int y = 0; y < h; y++) {
                ...
                if (binnedDS[x][y] > 0) {
                    g.setColor(new Color(s.c.getRed(), s.c.getGreen(), s.c.getBlue(), 200));// (int) Math.min(255, ((binnedDS[x][y] / maxSumBin) * 255))));
                    g.fillOval(x, y, 2, 2);
                }
            }
        }
    ...
    public BufferedImage generateImage() {
        ...
        final boolean ASINH_SCALE = true;
        if (ASINH_SCALE) {
            int maxLog = 6;
            double prevX = 30;
            double prevY = h - 30;
    ...
    public static class Series {
        ...
        public Series(String name, Color c, double[][] data, double smoothing, Type type) {
