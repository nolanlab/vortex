 ...
 * @author Nikolay
 */
class ClusterBoxElement extends JLabel implements zOrdered {
    ...
    public void setAnnotation(ColorPalette palette, Annotation ann) {
        ...
        for (String classLabel : ann.getTerms()) {
            ...
            int classSize = 0;
            String[] pids = ann.getProfileIDsForTerm(classLabel);
            for (String pid : pids) {
                if (ds.getDPbyName(pid) != null) {
                    if (cluster.containsDpID(ds.getDPbyName(pid).getID())) {
                        classSize++;
                    }
                }
            }
