        ...
        for (int sz = 10; sz <= 20; sz += 10) {
            ...
            for (int i = 0; i < SIZES.length; i++) {
                for (int j = i + 1; j < SIZES.length; j++) {
                    ...
                    if (MatrixOp.lenght(diff) < (4.5 + (sz / 20.0))) {
                        sz -= 10;
            ...
            for (int i = 0; i < SIZES.length; i++) {
                ...
                gen[i] = new MultinormalPCAGen(new NormalGen(new MRG31k3p()), mu[i].toArray(), sigma[i].toArray());//new MultivariateStudentGen(mu[i].toArray(), sig, 3+z);
            }
            int sumSizes = 0;
                ...
                for (int j = 0; j < SIZES[i]; j++) {
                   //if(i==1) return;
                    ...
                    for (int k = 0; k < SIZES.length; k++) {
                        dens += /*((MultinomialStudentGenerator)gen[k])*/ dist[k].density(vec) * (SIZES[k] / (double) sumSizes);
                    }
                  //  logger.print(Arrays.toString(vec));
                    Datapoint d = new Datapoint("c_" + i + "_" + j, vec, new double[]{dens}, dp.size());
                    dp.add(d);
                }
            }
