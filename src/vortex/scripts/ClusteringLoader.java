    ...
    public static ClusterSet[] loadFromSPADE(File dir, Dataset d, int[] freeParam) throws FileNotFoundException, IOException {
        ...
        for (int i = 0; i < freeParam.length; i++) {
            ...
            for (int j = 0; j < fcs.getEventCount(); j++) {
                ...
                if (Integer.parseInt(d.getDatapoints()[j].getName().split("Event")[1].trim()) != j) {
                    logger.print("Error: " + d.getDatapoints()[j].getName() + " doesnt match " + j);
                }
                //logger.print(j, cid);
            ...
            for (int j = 0; j < clusters.length; j++) {
                if (clusters[j] == null) {
                    ...
                    cl.add(new Cluster(clusters[j].toArray(new Datapoint[clusters[j].size()]), new double[d.getDimension()], new double[d.getSideVarNames().length], "", new EuclideanDistance()));
                }
            }
