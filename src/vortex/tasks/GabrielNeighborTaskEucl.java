...
public class GabrielNeighborTaskEucl extends GabrielNeighborTaskAngular {
    ...
    public int[] execute() {
        ...
        int maxIdx = Math.min(ds.getDimension() * 100, sorted.size());
        ...
        for (int y = 0; y < maxIdx; y++) {
            ...
            mid = MatrixOp.sum(center, anotherVector);
            MatrixOp.mult(mid, 0.5);

            double distBetween = Math.max(MatrixOp.getEuclideanDistance(mid, center), MatrixOp.getEuclideanDistance(mid, anotherVector));
//            assert MatrixOp.mult(mid, center) == Kernel.getEuclideanCosine(new DenseDoubleMatrix1D(mid), new DenseDoubleMatrix1D(center));
            ...
            if (false && YisNeighbor && nnOfTheMid != null) {
                ...
                b = MatrixOp.getEuclideanDistance(anotherVector, midShiftedToNN);
