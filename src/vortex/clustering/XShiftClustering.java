 ...
 * @author Nikolay
 */
public class XShiftClustering extends ClusteringCore {
    ...
    public static Cluster mergeClusters(Cluster a, Cluster b) {
        try {
            ...
            if (alCl != null) {
                ...
                if (!alCl.isEmpty()) {
                    return new Cluster(alCl.toArray(new Datapoint[alCl.size()]), paMain.getAverage(), pa.getAverage(), alCl.get(0).getName());
                }
            }
    ...
    public Cluster[] doClustering(final Dataset ds, final int K, final int[][] sortedNeighborLists, int nSize) {
        ...
        for (int idx = 0; idx < roots.size(); idx++) {
            ...
            if (alCl != null) {
                ...
                if (!alCl.isEmpty()) {
                    Cluster c = new Cluster(alCl.toArray(new Datapoint[alCl.size()]), paMain.getAverage(), pa.getAverage(), roots.get(idx).getName(), dm);
                    clusters.add(c);
                }
