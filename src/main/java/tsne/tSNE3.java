...
//	    p = VectorTools.mult(VectorTools.getAdd(p, VectorTools.transpose(p)), .5);
        ...
        for (int iter = 0; iter < max_iter; iter++) {
            ...
            double[][] sum_ydata = VectorTools.to2D(VectorTools.sumRow(VectorTools.fast_pow(ydata, 2)));
            //sum_ydata = sum(ydata .^ 2, 2);

            //new double[1][1];
            //num = 1 ./ (1 + bsxfun(@plus, sum_ydata, bsxfun(@plus, sum_ydata', -2 * (ydata * ydata')))); % Student-t distribution
            double[][] inner = VectorTools.mult(VectorTools.mult(ydata, VectorTools.transpose(ydata)), -2);
            double num[][] = VectorTools.addAsRow(inner, VectorTools.transpose(sum_ydata));
            ...
            inner = VectorTools.pointMult(VectorTools.getAdd(gains, 0.2), VectorTools.notEqual(VectorTools.sign(y_grads), VectorTools.sign(y_incs)));
            double[][] inner2 = VectorTools.pointMult(VectorTools.mult(gains, 0.8), VectorTools.equal(VectorTools.sign(y_grads), VectorTools.sign(y_incs)));
