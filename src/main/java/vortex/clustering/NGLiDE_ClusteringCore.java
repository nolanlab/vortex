...
public class NGLiDE_ClusteringCore extends KernelClusteringCore {
     ...
     */
    private double getvMFDens(double[] center, double[][] ArrayLists, double bandwidth) {
        ...
        dens *= NC / (double) ArrayLists.length;
        return dens;
    }
...
//        final double maxGrad = 1.000000;
        ...
        for (int x = 0; x < len; x++) {
            ...
            SwingWorker<Datapoint, Datapoint> sw = new SwingWorker<Datapoint, Datapoint>() {
                ...
                protected Datapoint doInBackground() throws Exception {
                    ...
                    if (parent == null) {
                        roots.add(currDp);
                        logger.print("Point# " + currDp.getID() + ", " + currDp.getFullName() + " is a root #" + (roots.size()));
                    }
                    return currDp;
                }
            };
        ...
        for (Datapoint root : roots) {
            ...
            if (alCl != null) {
                ...
                double[][] clusterArrayLists = new double[dpMembership.length][];

                ProfileAverager pa = new ProfileAverager();

                for (int i = 0; i < clusterArrayLists.length; i++) {
                    clusterArrayLists[i] = dpMembership[i].getKey().getVector();
                    pa.addProfile(dpMembership[i].getKey().getSideVector());
                }
                if (clusterArrayLists.length > 0) {
                    double[] rtVec = root.getVector();
                    try {
                        rtVec = kernel.getWeightedMean(root.getVector(), clusterArrayLists);
                    } catch (Exception e) {
                    }
                    final double[] refinedRoot = rtVec;
                    Arrays.sort(clusterArrayLists, new Comparator<double[]>() {
                        @Override
                    ...
                    Cluster c = new Cluster(dpMembership, refinedRoot, pa.getAverage(), root.getFullName(), dm);
                    clusters.add(c);
                }
            }
        }
