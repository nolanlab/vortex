 ...
 * @author Nikolay
 */
public class ImportClustering extends ClusteringAlgorithm {
    ...
    public ClusterSet[] doBatchClustering(Dataset ds, String comment) {
        ...
        for (int i = 0; i < css.length; i++) {
            ...
            for (int k = 0; k < pidLists.size(); k++) {
                ...
                ArrayList<Datapoint> alDP = new ArrayList<>();

                double[][] vec = new double[pids.length][];
                double[][] sidevec = new double[pids.length][];
                for (int j = 0; j < pids.length; j++) {
                    Datapoint dp = ds.getDPByID(Integer.parseInt(pids[j]));
                    if (dp == null) {
                        try {
                            dp = ds.getDPByID(Integer.parseInt(pids[j]));
