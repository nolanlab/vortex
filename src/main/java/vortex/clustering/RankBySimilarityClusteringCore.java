 ...
 * @author Nikolay
 */
public class RankBySimilarityClusteringCore extends ClusteringAlgorithm {
    ...
    public ClusterSet[] doBatchClustering(Dataset ds, String comment) {
        ...
        for (Entry<String, double[]> mode : modes) {
            if (mode.getValue().length != ds.getNumDimensions()) {
                throw new IllegalArgumentException("The dimensionality of the provided profile (" + mode.getValue().length + ") doesn't match the dimensiolality of the dataset (" + ds.getNumDimensions() + ")");
            }
