...
public class vMFKernel extends Kernel {
    ...
    public void fitBandwidth(double[] center, double[][] ArrayLists) {
        ...
        for (int i = 0; i < 500; i++) {
            ...
            double sumLogWeight = Math.log(getVMFConst(center.length, bandwidth))*ArrayLists.length;
            
            for (double[] v : ArrayLists) {
    ...
    public double getValue(double[] center, double[] ArrayList) {
        ...
        return getVMFConst(center.length, bandwidth) * exp(MatrixOp.getEuclideanCosine(center, ArrayList) * bandwidth);
    }
    ...
    protected double getNonNormalizedValue(double[] center, double[] ArrayList) {
        ...
        return exp(bandwidth * MatrixOp.getEuclideanCosine(ArrayList, center));
    }
    ...
    public BigDecimal getDensity(double[] center, double[][] ArrayLists) {
        ...
        for (int i = 0; i < ArrayLists.length; i++) {
            val = Math.exp(bandwidth * ((MatrixOp.mult(center, ArrayLists[i]) / MatrixOp.lenght(ArrayLists[i]))));
