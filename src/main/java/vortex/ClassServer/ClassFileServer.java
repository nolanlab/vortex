...
public class ClassFileServer extends ClassServer {
     ...
     * two optional command line arguments, the port on which the server accepts
     * requests and the root of the classpath. To start up the server: <br><br>
     *
     ...
     * classpath is the same as the RMI server's classpath): <br><br>
     *
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            logger.print("Unable to start ClassServer: " + e.getMessage());
            e.printStackTrace();
        }
    }
     ...
     * @exception ClassNotFoundException if the class corresponding to
     * <b>path</b> could not be loaded.
     */
    public byte[] getBytes(String path) throws java.io.IOException, ClassNotFoundException {
        ...
        if (codebases == null) {
            ...
            for (int i = 0; i < codebases.length && b == null; i++) {
                try {
                    ...
                    if (b != null) {
                        logger.print("found the class " + path + " in " + codebases[i]);
                    }
                } catch (java.io.IOException e) {
                }
            }
        ...
        if (b != null) {
            ...
            logger.print("!!! FAILED TO FIND THE CLASS " + path);
        }
     ...
     *
     * @param path the fqn of the class
     * @param codeBase the File that must be a jar or zip archive that may
     * contain the class
     * @return the bytecodes for the class
     * @exception java.io.IOException if the class cannot be read
     */
    private byte[] getBytesFromArchive(String path, java.io.File archive)
        ...
        //if (logger.isDebugEnabled()) {
        // //logger.debug("ClassFileServer reading: " + filename+" length="+length+" from jar/xip file rchive.getAbsolutePath());
        //}
        if (length == -1) {
            throw new java.io.IOException("File length is unknown: "
                    + filename);
     ...
     *
     * @param path the fqn of the class
     * @param codeBase the File that must be a directory that may contain the
     * class
     * @return the bytecodes for the class
     * @exception java.io.IOException if the class cannot be read
     */
