...
//logger.print("Filtering  by basal1");
            for (String term : ann.getTerms()) {
                ...
                for (int i = 0; i < classifiedLists.length; i++) {
                    c[i] = new Cluster(classifiedLists[i].toArray(new Datapoint[classifiedLists[i].size()]), new double[ds.getNumDimensions()], new double[ds.getSideVarNames().length], term, new AngularDistance());
                    c[i].setID(i);
                }
